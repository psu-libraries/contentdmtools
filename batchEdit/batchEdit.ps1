...
ForEach ($header in $headers) {
 Import-Csv tmp.csv | Sort-Object * -Unique | Where-Object {$_.field -eq "$header"} | Where-Object {$_.field -ne "cdmnumber"} | Export-Csv ("metadata_edited_" + $header + ".csv") -NoTypeInformation
}
...
Get-ChildItem -Filter metadata_edited_* | ForEach-Object {
