...
{
  $directories.$directory |Export-Csv -Delimiter "`t" -Path $source\$directory\$directory.txt -NoTypeInformation  | Tee-Object -file $log
}
...
if($abbyy)
{
  ...
  robocopy . $abbyyStage $source\*.tif* /S | Out-Null
  ...
  DO {
    $count_staged = Get-ChildItem -Path $source -Include *.tif* -Recurse -File -Force | Measure-Object | ForEach-Object {$_.Count}
    $count_txt = Get-ChildItem -Path "$abbyyOutput" -Include *.txt -Recurse -File -Force | Measure-Object | ForEach-Object {$_.Count}
    $count_jp2 = Get-ChildItem -Path "$abbyyOutput" -Include *.jp2 -Recurse -File -Force | Measure-Object | ForEach-Object {$_.Count}
   ...
   Move-Item -Path "$abbyyOutput\*" -Destination $source  | Tee-Object -file $log
   Start-Sleep 30
...
if($im){
  ...
  Get-ChildItem -Path $source -Include *.tif* -Recurse | ForEach-Object { magick.exe convert -quiet ($_.FullName + "[0]") "$($_.FullName -Replace "tif", "jp2")" } | Tee-Object -file $log
...
if($deltif){
  ...
  Get-ChildItem -Path $source -Include *.tif* -Recurse | ForEach-Object { Remove-Item -Path $_.FullName } | Tee-Object -file $log
