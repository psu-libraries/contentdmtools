...
<#
    ...
    .PARAMETER ocrengine
    Options for OCR engine: ABBYY and tesseract. ABBYY) Penn State licensed commercial software, Tesseract) Open-source OCR software. (DEFAULT VALUE: Tesseract)
    .PARAMETER originals
    ...
    .PARAMETER throttle
    Integer for the number of CPU processes when parallel processing, such as image conversion, OCR, and copying files to the ABBYY server. (DEFAULT VALUE: 4)
    .EXAMPLE
    .\batchCreate.ps1 -path E:\pstsc_01822\2018-02\ -jp2 no -ocr extract -ocrengine tesseract -originals discard -throttle 4
    .INPUTS
    System.String
    System.Integer
    .NOTES
...
ForEach ($object in $objects) {
    ...
    if ($jp2 -eq "true") {
        ...
        Get-ChildItem -Path $path\$object -Filter "*.tif*" | ForEach-Object -ThrottleLimit $throttle -Parallel {
